...
manyPlayersManyMoves game moves = playMoves game moves (getPlayers game)

playMoves :: [[Char]] -> [Int] -> [Char] -> [[Char]]
playMoves game [] _ = game
playMoves game moves [] = playMoves game moves (getPlayers game)
playMoves game (move:moves) (player:players)
    | winConnectFour game = game
    | otherwise = playMoves (playerMoveOnce game (move - 1) player) moves players
...
getPlayers :: [[Char]] -> [Char] -- Quinn gave this to me
getPlayers game = dropWhile (== '-') (sort (nub (concat game)))
